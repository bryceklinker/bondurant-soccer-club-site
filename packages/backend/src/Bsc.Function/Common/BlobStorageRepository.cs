    ...
    where T : class
{
    ...
    public BlobContainerClient ContainerClient => _serviceClient.GetBlobContainerClient(ContainerName);
    ...
    public async Task<T[]> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using var readStream = await BlobClient.OpenReadAsync(cancellationToken: cancellationToken).ConfigureAwait(false);
        var serializerResult = await BscSerializer.DeserializeAsync<T[]>(readStream, cancellationToken).ConfigureAwait(false);

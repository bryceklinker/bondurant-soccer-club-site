...
public class FakeBlobContainerClient : BlobContainerClient
{
    ...
    public override Task<Response<BlobContainerInfo>> CreateIfNotExistsAsync(PublicAccessType publicAccessType = PublicAccessType.None, IDictionary<string, string> metadata = null,
        BlobContainerEncryptionScopeOptions encryptionScopeOptions = null,
        CancellationToken cancellationToken = new CancellationToken())
    {
        var json = BlobContainerInfoTemplate;
        var response = Response.FromValue<BlobContainerInfo>(JsonConvert.DeserializeObject<BlobContainerInfo>(json), new FakeResponse());

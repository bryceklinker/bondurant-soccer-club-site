...
import {MAIN_NAVIGATION_LINKS, MORE_INFO_NAVIGATION_LINKS} from '../../../routing/route-names';

export function useVisibleLinks(links: LinkData[]) {
    const isLoggedIn = useIsLoggedIn();
    return useMemo(() => links.filter(
...
export function useAllNavigationLinks() {
    ...
    return useMemo(() => mainNavLinks.concat(moreNavLinks), [mainNavLinks, moreNavLinks]);
}
